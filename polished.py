...
class ProductTrader:
    ...
    def __init__(self, name, state, prints, new_trader_data, product_group=None):
        ...
        self.initial_position = self.state.position.get(
            self.name, 0)  # position at beginning of round

        # update this if you expect a certain change in position e.g. to already hedge
    ...
    def get_total_market_buy_sell_volume(self):
        ...
        try:
            market_bid_volume = sum(
                [v for p, v in self.mkt_buy_orders.items()])
            market_ask_volume = sum(
                [v for p, v in self.mkt_sell_orders.items()])
        except:
            pass
    ...
    def get_order_depth(self):
        ...
        try:
            ...
            pass
        try:
            buy_orders = {bp: abs(bv) for bp, bv in sorted(
                order_depth.buy_orders.items(), key=lambda x: x[0], reverse=True)}
        except:
            pass
        try:
            sell_orders = {sp: abs(sv) for sp, sv in sorted(
    ...
    def bid(self, price, volume, logging=True):
        ...
        if logging:
            self.log("BUYO", {"p": price, "s": self.name,
                     "v": int(volume)}, product_group='ORDERS')
    ...
    def ask(self, price, volume, logging=True):
        ...
        order = Order(self.name, int(price), -abs_volume)
        if logging:
            self.log("SELLO", {"p": price, "s": self.name,
                     "v": int(volume)}, product_group='ORDERS')
    ...
    def check_for_informed(self):
        ...
        informed_bought_ts, informed_sold_ts = self.last_traderData.get(self.name, [
                                                                        None, None])

        trades = self.state.market_trades.get(
            self.name, []) + self.state.own_trades.get(self.name, [])
        ...
        self.new_trader_data[self.name] = [
            informed_bought_ts, informed_sold_ts]
...
class EtfTrader:
    def __init__(self, state, prints, new_trader_data):
        ...
        self.baskets = [ProductTrader(
            s, state, prints, new_trader_data, product_group='ETF') for s in ETF_BASKET_SYMBOLS]
        self.informed_constituent = ProductTrader(
            ETF_INFORMED_CONSTITUENT, state, prints, new_trader_data, product_group='ETF')
        self.hedging_constituents = [ProductTrader(s, state, prints, new_trader_data, product_group='ETF')
    ...
    def calculate_spread(self, basket):
        ...
        try:
            ...
            constituents = [self.informed_constituent] + \
            ...
            index_price = np.asarray(
                const_prices) @ np.asarray(ETF_CONSTITUENT_FACTORS[b_idx])
            ...
            if CALCULATE_RUNNING_ETF_PREMIUM:
                ...
                try:
                    ...
                    basket.log(f'ETF_{b_idx}_IDXP', round(
                        index_price + mean_premium, 2))
    ...
    def get_constituent_orders(self):
        ...
        remaining_volume = expected_position - \
            self.informed_constituent.initial_position
        ...
        for hedging_constituent in self.hedging_constituents:
            ...
            for b_idx, basket in enumerate(self.baskets):
                etf_const_factor = ETF_CONSTITUENT_FACTORS[b_idx][ETF_CONSTITUENT_SYMBOLS.index(
                    hedging_constituent.name)]
                expected_hedge_position += -basket.expected_position * \
                    etf_const_factor * ETF_HEDGE_FACTOR

            remaining_volume = round(
                expected_hedge_position - hedging_constituent.initial_position)

            if remaining_volume > 0:
                hedging_constituent.bid(
                    hedging_constituent.ask_wall, remaining_volume)

            elif remaining_volume < 0:
                hedging_constituent.ask(
                    hedging_constituent.bid_wall, -remaining_volume)
...
class OptionTrader:
    def __init__(self, state, prints, new_trader_data):
        ...
        self.options = [ProductTrader(
            os, state, prints, new_trader_data, product_group='OPTION') for os in OPTION_SYMBOLS]
        self.underlying = ProductTrader(
            OPTION_UNDERLYING_SYMBOL, state, prints, new_trader_data, product_group='OPTION')
        ...
        if self.underlying.wall_mid is not None:
            ...
            for option in self.options:
                ...
                if option.wall_mid is not None:
                    ...
                    tte = 1 - (DAYS_PER_YEAR - 8 + DAY +
                               self.state.timestamp // 100 / 10_000) / DAYS_PER_YEAR
            ...
            if option.name in self.indicators['mean_theo_diffs'] and option.name in self.indicators['current_theo_diffs'] and option.name in self.new_switch_mean:
                ...
                if self.new_switch_mean[option.name] >= IV_SCALPING_THR:
                    ...
                    if current_theo_diff - option.wall_mid + option.best_bid - mean_theo_diff >= (THR_OPEN + low_vega_adj) and option.max_allowed_sell_volume > 0:
                        option.ask(option.best_bid,
                                   option.max_allowed_sell_volume)
                    ...
                    if current_theo_diff - option.wall_mid + option.best_bid - mean_theo_diff >= THR_CLOSE and option.initial_position > 0:
                        ...
                        option.bid(option.best_ask,
                                   option.max_allowed_buy_volume)
    ...
    def get_mr_orders(self, options):
        ...
        for option in options:
            ...
            if option.name in self.indicators['current_theo_diffs'] and option.name in self.indicators['mean_theo_diffs'] and self.indicators.get('ema_o_dev') is not None:
                ...
                iv_deviation = self.indicators['current_theo_diffs'][option.name] - \
    ...
    def get_option_orders(self):
        ...
        if self.state.timestamp / 100 < min([THEO_NORM_WINDOW, underlying_mean_reversion_window, options_mean_reversion_window]):
            return {}

        iv_scalping_options = [o for o in self.options if int(
            o.name.split('_')[-1]) >= 9750]
    ...
    def get_underlying_orders(self):
        ...
        if self.indicators.get('ema_u_dev') is not None:
            ...
            if current_deviation > underlying_mean_reversion_thr and self.underlying.max_allowed_sell_volume > 0:
                self.underlying.ask(
                    self.underlying.bid_wall + 1, self.underlying.max_allowed_sell_volume)
...
class CommodityTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        self.log('M_ARBS', [round(mean_long_arb_hist, 2),
                 round(mean_short_arb_hist, 2)])
