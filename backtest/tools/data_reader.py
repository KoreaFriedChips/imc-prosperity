...
from backtest.datamodel import Trade, Symbol
from backtest.models.input import PriceRow, ObservationRow, BacktestData
...
class PackageResourcesReader(BackDataReader):
    def _read_file_content(self, path_parts: list[str]) -> ContextManager[Optional[Path]]:
        try:
            file_path = f"backtest.resources.{'.'.join(path_parts[:-1])}"
